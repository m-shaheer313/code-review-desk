...
| T108 | **Dropped.** ~~Implement `list_changed_files` and `get_file_diff` tools, backed by the chunks from T103~~ — never built: reviewers receive the full reviewable diff text directly as their run input, so no per-file lookup tool is needed (see plan.md §3, "No per-file lookup tools") | — | n/a | — |
...
| T205 | Configure the Style Reviewer's `get_ruleset` tool call as **required** via forced tool choice — as a separate lookup step before the findings step, because Gemini rejects a forced call combined with JSON output (plan.md §9, revised 2026-09-25) | FR-9a | a Style review without the ruleset tool being called is impossible — verify by inspecting the lookup step's model-facing tool config, by `tests/test_fr9.py` (no agent combines a forced choice with structured output; lookup always runs first and its ruleset reaches the review), and live via `scripts/live_test_fr9a.py` (passed on gemini-3.6-flash, 2026-09-25) | T107, T201 |
| T206 | Confirm `get_ruleset` — the only tool any reviewer has (T108 dropped) — hands its failures to its own internal error handling, never raising into the runner | FR-9b, Art. III | unknown, empty and traversal-shaped ruleset ids, a missing context, and an unreadable, undecodable, empty or deleted ruleset file all produce a clean "ruleset unavailable" string, not an exception, at the tool level (`tests/test_fr9.py`, 9b section) | T107 |
